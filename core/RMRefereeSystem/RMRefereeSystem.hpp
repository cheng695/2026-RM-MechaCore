 ...
 */
class RMRefereeSystemConnector
{
public:
    RMRefereeSystemConnector(uint32_t timeout_ms = 1000)
        : statewatch_(timeout_ms) {}

    /** 喂狗：在解析到有效帧时调用 */
    void Feed() { statewatch_.UpdateLastTime(); }

    /** 查询是否在线（内部自动刷新时间并判定超时） */
    bool IsConnected()
    {
    ...
    /** 获取底层状态 */
    BSP::WATCH_STATE::Status GetStatus() const { return statewatch_.GetStatus(); }

private:
    BSP::WATCH_STATE::StateWatch statewatch_;
};

extern RMRefereeSystemConnector RMRefereeSystemConnection;  // 全局断连检测实例
