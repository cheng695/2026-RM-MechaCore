...
// 获取跟踪信号
float TDFilter::getTrackingSignal() const
{
    return v1;
}
...
filteredData = td.filter(filteredData);
filteredData = lpf.filter(filteredData);
...
// TD跟踪微分器状态获取
float tdTracking = td.getTrackingSignal();    // 获取跟踪信号
