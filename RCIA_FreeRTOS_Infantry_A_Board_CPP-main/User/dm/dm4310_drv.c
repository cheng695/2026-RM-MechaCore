...
void dm4310_fbdata(motor_t *motor, uint8_t *rx_data)
{
	...
	motor->para.pos_vision = uint_to_float(motor->para.p_int, P_MIN, P_MAX, 16) * 114.591; // (-12.5,12.5)
	motor->para.pos = uint_to_float(motor->para.p_int, P_MIN, P_MAX, 16) * 2607.6; // (-12.5,12.5)
...
void pos_speed_ctrl(hcan_t* hcan,uint16_t motor_id, float pos, float vel)
{
    ...
    uint32_t pos_bytes = *(uint32_t*)&pos;
    uint32_t vel_bytes = *(uint32_t*)&vel;
    data[0] = (pos_bytes >> 0) & 0xFF;  // LSB first
    data[1] = (pos_bytes >> 8) & 0xFF;
    data[2] = (pos_bytes >> 16) & 0xFF;
    data[3] = (pos_bytes >> 24) & 0xFF;
    data[4] = (vel_bytes >> 0) & 0xFF;
    data[5] = (vel_bytes >> 8) & 0xFF;
    data[6] = (vel_bytes >> 16) & 0xFF;
    data[7] = (vel_bytes >> 24) & 0xFF;;
