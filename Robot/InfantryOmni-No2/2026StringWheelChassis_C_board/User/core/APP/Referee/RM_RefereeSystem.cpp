...
// 死亡时间
BSP::WATCH_STATE::StateWatch dirTime(1000);
// 断连标记
...
void RM_RefereeSystemDelete(const char operate, const char number)
{
    ...
    RM_RefereeSystemDataTemp.seq = seq++;
...
void RM_daohangtx(const map_robot_data_t map_robot_data)
{
    ...
    // 注意：这里 rrrdata 和 tx_buf 好像是独立的 globals，但最好也 protected
    // 之前代码里没有明确发 rrrdata 的函数调用？假设这里是漏了 HAL_UART_Transmit
    // 不过原代码确实没写 Transmit... 你可能需要确认下这里是不是写了一半？
    // 为了保险，我先假设你是要在外部发，或者这里漏了。
    // 但既然你让我改 Refereee，我就顺手把 Transmit 补上吧，或者至少把 seq 加上
    
    HAL_UART_Transmit_DMA(&RM_RefereeSystemHuart, rrrdata, sizeof(rrrdata)-5); // 稍微估算下长度，或者 sizeof(rrrdata)
...
// 消息获取（状态机模式）
void RM_RefereeSystemGetData(uint8_t byte)
{
    static uint8_t RxBuffer[256]; // 增加缓冲区大小
    ...
    case STATE_CRC8:
        RxBuffer[RxIndex++] = byte;
        // 校验帧头CRC8
        if (Verify_CRC8_Check_Sum(RxBuffer, 5))
        {
            State = STATE_CMD_ID_LOW;
        }
        else
        {
            State = STATE_SOF; // 校验失败，重新寻找帧头
        }
        break;

    case STATE_CMD_ID_LOW:
        RxBuffer[RxIndex++] = byte;
        State = STATE_CMD_ID_HIGH;
        break;

    case STATE_CMD_ID_HIGH:
        RxBuffer[RxIndex++] = byte;
        State = STATE_DATA;
        // 如果数据长度为0 (虽然很少见)，直接跳到尾部
        if (DataLen == 0)
        {
            State = STATE_TAIL_LOW;
        }
        break;

    case STATE_DATA:
        if (RxIndex < (sizeof(RxBuffer) - 2)) // 预留2字节给CRC16
        {
            RxBuffer[RxIndex++] = byte;
            // 头部5 + CmdID 2 + DataLen = 目前期望的索引位置
            if (RxIndex >= (5 + 2 + DataLen))
            {
                State = STATE_TAIL_LOW;
            }
        }
        ...
        {
            State = STATE_SOF; // 缓冲区溢出保护
        }
        break;

    case STATE_TAIL_LOW:
        RxBuffer[RxIndex++] = byte;
        State = STATE_TAIL_HIGH;
        break;

    case STATE_TAIL_HIGH:
        RxBuffer[RxIndex++] = byte;
        // 接收完毕，整包校验
        if (Verify_CRC16_Check_Sum(RxBuffer, RxIndex))
        {
            RM_RefereeSystemParseData(RxBuffer, RxIndex);
        }
        State = STATE_SOF;
        break;
...
bool RM_RefereeSystemDir()
{
    dirTime.UpdateTime();
    dirTime.CheckStatus();
    if (dirTime.GetStatus() == BSP::WATCH_STATE::Status::OFFLINE)
    {
        RM_RefereeSystemDirFlag = true;
        RM_RefereeSystemInit();
    }
    else
    {
        RM_RefereeSystemDirFlag = false;
    }
...
{
    // 更新断连时间
    dirTime.UpdateLastTime();
    // 获取解析数据
    RM_RefereeSystemGetData(*data);
}
} // namespace RM_RefereeSystem
