...
Alg::Feedforward::Gravity gravity_forward(0.25f, -90.0f);  // 重力前馈（所有模式 Pitch）
...
/* 控制器 ---------------------------------------------------------------------------------------------------*/
ALG::PID::PID yaw_angle_pid(20.0f, 0.0f, 0.0f, 12.5664f, 0.0f, 0.0f);        // yaw轴角速度pid（视觉模式）
ALG::PID::PID yaw_velocity_pid(0.0f, 0.0f, 1.0f, 16384.0f, 0.0f, 0.0f); // yaw轴角速度pid（内置速度给Kd）

ALG::PID::PID pitch_angle_pid(20.0f, 0.0f, 0.0f, 12.5664f, 0.0f, 0.0f);        // pitch轴角速度pid（视觉模式）
ALG::PID::PID pitch_velocity_pid(0.0f, 0.0f, 1.0f, 16384.0f, 0.0f, 0.0f); // pitch轴角速度pid（内置速度给Kd）
 ...
 * @brief 根据角度限制速度，速度限幅
 * 
 * @param current_angle_deg 陀螺仪角度
 * @param max 陀螺仪角度
 * @param min 陀螺仪角度
 * @param velocity_cmd 期望速度
 * @return float 
 */
// float LimitPitch2VelocityByAngle(float current_angle_deg, float max, float min, float velocity_cmd)
// {
//     if (current_angle_deg >= max && velocity_cmd > 0.0f)
//     {
//         return 0.0f;
//     }
//     if (current_angle_deg <= min && velocity_cmd < 0.0f)
//     {
//         return 0.0f;
//     }
//     return velocity_cmd;
    ...
    switch(gimbal_fsm.Get_Now_State()) 
    {
        ...
        case MANUAL:    // 普通模式
            if(DT7.get_s1() == 3 && DT7.get_s2() == 3)
            {
                ...
                gimbal_target.target_pitch += 0.004f * mouse_y_input;  // 弧度rad
                gimbal_target.target_pitch = std::clamp(gimbal_target.target_pitch, -0.3491f, 0.5236f);  // 弧度rad
            }
            else
            {
                gimbal_target.target_yaw += 0.01f * -DT7.get_right_x();  // 弧度rad
                gimbal_target.target_pitch += 0.007f * DT7.get_right_y(); // 弧度rad
                gimbal_target.target_pitch = std::clamp(gimbal_target.target_pitch, -0.3491f, 0.5236f);  // 弧度rad
            }
            break;
        case VISION:    // 视觉模式
...
void gimbal_stop()
{
    gimbal_output.motor_pitch_enable = false; // 通知 MotorTask 失能 4310 Pitch
    gimbal_output.motor_yaw_enable   = false; // 通知 MotorTask 失能 4310 Yaw
    // 重置控制器
...
void gimbal_manual()
{
    ...
    // Yaw 角度PID
    yaw_angle_pid.UpDate(gimbal_target.target_yaw, HI12.GetAddYaw() * 3.14159f / 180.0f); // 弧度rad
    // Yaw 输出，进行限幅
...
void gimbal_vision()
{
    ...
    // 过零处理，保证走最短路径 (适用于[-π, π]的突变)
    float current_yaw = HI12.GetAngle(2) * 3.14159f / 180.0f;
    float err_yaw = gimbal_target.target_yaw - current_yaw;

    while (err_yaw > 3.14159f)  err_yaw -= 6.28318f;
    while (err_yaw < -3.14159f) err_yaw += 6.28318f;
...
{   
    ...
    if (gimbal_fsm.Get_Now_State() != last_gimbal_state)
    {
        ...
        // 状态切换时将目标值置为当前陀螺仪角度，防止跳变
        gimbal_target.target_yaw   = HI12.GetAddYaw() * 3.14159f / 180.0f;
        gimbal_target.target_pitch = HI12.GetAngle(1) * 3.14159f / 180.0f;
...
{
    // 初始化蜂鸣器管理器；初始化状态机 (4310 初始失能由 MotorTask 处理)
