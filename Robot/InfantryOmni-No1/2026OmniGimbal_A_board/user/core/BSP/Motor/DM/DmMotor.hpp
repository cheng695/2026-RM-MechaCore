...
namespace BSP::Motor::DM
{
   ...
   protected:
        ...
        * @brief 构造函数
        */
       DMMotorBase(uint16_t Init_id, const uint8_t (&recv_ids)[N], const uint32_t send_ids, Parameters params)
   ...
   public:
       ...
       void Parse(const HAL::CAN::Frame &frame) override
       {
           for (uint8_t i = 0; i < N; ++i)
           {
               if (frame.id == init_address + recv_idxs_[i])
               {
                   ...
                   feedback_[i].id = pData[0] & 0x0F;
                   feedback_[i].err = (pData[0] >> 4) & 0x0F;
        ...
        * @brief DM电机的MIT控制方法
        */
       void ctrl_Mit(uint8_t motor_index, float _pos, float _vel, 
               float _KP, float _KD, float _torq)
       {
           ...
           send_data[7] = tor_tmp;
            ...
            HAL::CAN::Frame frame;
            frame.id = send_idxs_;  // MIT模式无偏移
            frame.dlc = 8;
            memcpy(frame.data, send_data, 8);
        ...
        * @brief DM电机的角度速度控制方法
        */
       void ctrl_AngleVelocity(uint8_t motor_index, float _pos, float _vel)
       {
           uint8_t data[8];
           uint8_t *pbuf, *vbuf;
           ...
           for (int i = 0; i < 4; ++i) 
           {
            ...
            HAL::CAN::Frame frame;
            frame.id = 0x100 + send_idxs_;  // 位置速度模式+0x100偏移
            frame.dlc = 8;
        ...
        * @brief DM电机的速度控制方法
        */
       void ctrl_Velocity(uint8_t motor_index, float _vel)
       {
           uint8_t data[8] = {0};
           ...
           for (int i = 0; i < 4; ++i) 
           {
            ...
            HAL::CAN::Frame frame;
            frame.id = 0x200 + send_idxs_;  // 位置速度模式+0x100偏移
            frame.dlc = 8;
            memcpy(frame.data, data, 8);
            frame.is_extended_id = false;
            frame.is_remote_frame = false;
         ...
         * @brief 使能电机
         * @note 帧ID根据当前模式决定
         * @param mode 0:MIT模式, 1:位置速度模式, 2:速度模式
         */
        void on(uint8_t motor_index, uint8_t mode = 0)
        {     
            uint8_t send_data[8] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFC};
            
            uint32_t frame_id = send_idxs_;
            // 根据模式添加偏移
            if (mode == 1) frame_id += 0x100;  // 位置速度模式
            else if (mode == 2) frame_id += 0x200;  // 速度模式
         ...
         * @param mode 0:MIT模式, 1:位置速度模式, 2:速度模式
         */
        void off(uint8_t motor_index, uint8_t mode = 0)
        {  
            uint8_t send_data[8] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFD};
            
            uint32_t frame_id = send_idxs_;
            if (mode == 1) frame_id += 0x100;
            else if (mode == 2) frame_id += 0x200;
        ...
        * @param mode 0:MIT模式, 1:位置速度模式, 2:速度模式
        */
      void clearError(uint8_t motor_index, uint8_t mode = 0)
        {
   ...
   {
   public:
       J4310(uint16_t Init_id, const uint8_t (&ids)[N], const uint32_t send_idxs)
           : DMMotorBase<N>(Init_id, ids, send_idxs, 
                           Parameters(-12.56f, 12.56f, -30.0f, 30.0f, -10.0f, 10.0f, 0.0f, 500.0f, 0.0f, 5.0f))
       {
       }
   };
   ...
   {
   public:
       S2325(uint16_t Init_id, const uint8_t (&ids)[N], const uint32_t send_idxs)
           : DMMotorBase<N>(Init_id, ids, send_idxs,
