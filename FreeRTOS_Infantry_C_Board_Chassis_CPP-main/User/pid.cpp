...
/// @param target 目标值
/// @param measure 实际值
/// @param feed_forward 前馈值
/// @return PID输出值
float PID::ComputeWithFeedForward(float target, float measure, float feed_forward) {
    error_ = target - measure;
    integral_ += error_;
    integral_ = clamp(integral_, -integral_limit_, integral_limit_);
    derivative_ = error_ - prev_error_;
    prev_error_ = error_;
    output_ = kp_ * error_ + ki_ * integral_ + kd_ * derivative_ + feed_forward;
    output_ = clamp(output_, -output_limit_, output_limit_);
    return output_;
}
