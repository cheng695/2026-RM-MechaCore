...
void Communicator::Send() {
	...
	tx_gimbal.vw = vw;  
	tx_gimbal.booster_heat_max = referee.robot_status.shooter_barrel_heat_limit;
	tx_gimbal.booster_heat_cd = referee.robot_status.shooter_barrel_cooling_value;
	tx_gimbal.booster_now_heat = referee.power_heat_data.shooter_17mm_1_barrel_heat;
	
	uint8_t len = sizeof(tx_gimbal);
	
	std::memcpy(tx_buf, &tx_gimbal, len);

//    AppendAccumulationCheckSum(tx_buf, sizeof(len));
    HAL_UART_Transmit_DMA(kCommUart, tx_buf, len);
}
